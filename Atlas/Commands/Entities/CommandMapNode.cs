...
namespace Atlas.Commands.Entities
{
    internal class CommandMapNode
    {
        ...
        public void AddCommand(CommandManager service, string text, int index, CommandInfo command)
        {
            int nextSegment = NextSegment(text, index, service.Config.SeparatorChar);
            string name;
            ...
            lock (_lockObj)
            {
                ...
                else
                {
                    ...
                    string fullName = _name == "" ? name : _name + service.Config.SeparatorChar + name;
        ...
        public void RemoveCommand(CommandManager service, string text, int index, CommandInfo command)
        {
            int nextSegment = NextSegment(text, index, service.Config.SeparatorChar);
        ...
        public IEnumerable<CommandMatch> GetCommands(CommandManager service, string text, int index, bool visitChildren = true)
        {
            ...
            if (visitChildren)
            {
                ...
                nextSegment = NextSegment(text, index, WhitespaceChars, service.Config.SeparatorChar);
                if (nextSegment != -1)
                {
                    name = text.Substring(index, nextSegment - index);
                    if (_nodes.TryGetValue(name, out nextNode))
